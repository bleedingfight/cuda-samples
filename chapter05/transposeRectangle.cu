...
__global__ void warmup(float *out, float *in, const int nrows, const int ncols)
{
...
int main(int argc, char **argv)
{
    ...
    warmup<<<grid, block>>>(d_C, d_A, nrows, ncols);
    ...
    CHECK(cudaDeviceSynchronize());
    elaps = chrono::duration_cast<chrono::nanoseconds>(chrono::high_resolution_clock::now()-start).count();

    CHECK(cudaMemcpy(gpuRef, d_C, nBytes, cudaMemcpyDeviceToHost));

    if(iprint) printData(gpuRef, nrows * ncols);

    auto size = 2 * ncells * sizeof(float);
    float ibnd = bandwidth(size, elaps);
    
    printf("copyGmem elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> "
           "effective bandwidth %f GB\n", elaps, grid.x, grid.y, block.x,
           block.y, ibnd);
    ...
    ibnd = bandwidth(size, elaps);
    printf("naiveGmem elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> "
           "effective bandwidth %f GB\n", elaps, grid.x, grid.y, block.x,
           block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("naiveGmemUnroll elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> "
           "effective bandwidth %f GB\n", elaps, grid2.x, grid2.y, block.x,
           block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmem elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> "
           "effective bandwidth %f GB\n", elaps, grid.x, grid.y, block.x,
           block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmemPad elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> "
           "effective bandwidth %f GB\n", elaps, grid.x, grid.y, block.x,
           block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmemDyn elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> "
           "effective bandwidth %f GB\n", elaps, grid.x, grid.y, block.x,
           block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmemPadDyn elapsed %ld nano sec <<< grid (%d,%d) block "
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmemUnroll elapsed %ld nano sec <<< grid (%d,%d) block "
           "(%d,%d)>>> effective bandwidth %f GB\n", elaps, grid2.x, grid2.y,
           block.x, block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmemUnrollPad elapsed %ld nano sec <<< grid (%d,%d) block "
           "(%d,%d)>>> effective bandwidth %f GB\n", elaps, grid2.x, grid2.y,
           block.x, block.y, ibnd);
    ...
    checkResult(hostRef, gpuRef, ncols, nrows);
    ibnd = bandwidth(size, elaps);
    printf("transposeSmemUnrollPadDyn elapsed %ld nano sec <<< grid (%d,%d) block "
           "(%d,%d)>>> effective bandwidth %f GB\n", elaps, grid2.x, grid2.y,
           block.x, block.y, ibnd);

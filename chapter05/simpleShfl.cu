...
__global__ void test_shfl_broadcast(int *d_out, int *d_in, int const srcLane) {
    int value = d_in[threadIdx.x]; // 此行之后16个线程已经将值分别拿出
...
__global__ void test_shfl_wrap(int *d_out, int *d_in, int const offset) {
    ...
    value = __shfl_sync(0xffffffff, value, threadIdx.x + offset,
                        BDIMX); // 将每个线程当前的值value给到其前2得到线程
    d_out[threadIdx.x] = value;
}

__global__ void test_shfl_wrap_plus(int *d_out, int *d_in, int const offset) {
    int value = d_in[threadIdx.x];
    value += __shfl_sync(value, threadIdx.x + offset, BDIMX);
    d_out[threadIdx.x] = value;
}
...
__global__ void test_shfl_down(int *d_out, int *d_in,
                               unsigned int const delta) {
    int value = d_in[threadIdx.x];
...
__inline__ __device__ void swap_old(int *value, int tid, int mask, int srcIdx,
                                    int dstIdx) {
...
__inline__ __device__ void swap(int *value, int laneIdx, int mask, int firstIdx,
                                int secondIdx) {
    ...
    value[secondIdx] =
        __shfl_xor_sync(0xffffffff, value[secondIdx], mask, BDIMX);
...
__global__ void test_shfl_swap(int *d_out, int *d_in, int const mask,
...
int main(int argc, char **argv) {
    ...
    if (iPrintout) {
        printf("shfl wrap left(offset=2)      : ");
        printData(h_outData, nElem);
    }
    ...
    if (iPrintout) {
        printf("shfl wrap right(offset=-2)    : ");
        printData(h_outData, nElem);
    }

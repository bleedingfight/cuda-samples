...
int main(int argc, char **argv) {
    ...
    long atomic_mean_time = 0l;
    long unsafe_mean_time = 0l;
    ...
    for (r = 0; r < runs; r++) {
        ...
        atomic_mean_time += DURATION_NANO(start_atomic);
        ...
        unsafe<<<N / block, block>>>(d_shared_var, d_values_read_unsafe, N,
                                     iters);
        CHECK(cudaDeviceSynchronize());
        unsafe_mean_time += DURATION_NANO(start_unsafe);
        CHECK(cudaMemcpy(&h_shared_var_unsafe, d_shared_var, sizeof(int),
                         cudaMemcpyDeviceToHost));
    }

    printf("In total, %d runs using atomic operations took %ld ns\n", runs,
           atomic_mean_time);
    printf("  Using atomic operations also produced an output of %d\n",
           h_shared_var_atomic);
    printf("In total, %d runs using unsafe operations took %ld ns\n", runs,
           unsafe_mean_time);

...
int main(int argc, char **argv) {
    ...
    long mean_intrinsic_time = 0l;
    long mean_standard_time = 0l;

    for (i = 0; i < runs; i++) {
        auto start_standard = std::chrono::high_resolution_clock::now();
        standard_kernel<<<1, 32>>>(input_value, d_standard_out, iters);
        CHECK(cudaDeviceSynchronize());
        mean_standard_time += DURATION_NANO(start_standard);

        auto start_intrinsic = std::chrono::high_resolution_clock::now();
        intrinsic_kernel<<<1, 32>>>(input_value, d_intrinsic_out, iters);
        CHECK(cudaDeviceSynchronize());
        mean_intrinsic_time += DURATION_NANO(start_intrinsic);
    }
    ...
    printf("\n");
    printf("Mean execution time for standard function powf:    %ld ns\n",
           mean_standard_time);
    printf("Mean execution time for intrinsic function __powf: %ld  ns\n",
           mean_intrinsic_time);

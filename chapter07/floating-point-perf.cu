...
static void run_float_test(size_t N, int niters, int blocksPerGrid,
                           int threadsPerBlock, long int *toDeviceTime,
                           long int *kernelTime, long int *fromDeviceTime,
                           float *sample, int sampleLength)
{
    ...
    auto toDeviceStart = chrono::high_resolution_clock::now();
    CHECK(cudaMemcpy(d_floatInputs, h_floatInputs, sizeof(float) * N,
                     cudaMemcpyHostToDevice));
    *toDeviceTime = DURATION_NANO(toDeviceStart);
    ...
    *kernelTime = DURATION_NANO(kernelStart);

    auto fromDeviceStart = chrono::high_resolution_clock::now();
    CHECK(cudaMemcpy(h_floatOutputs, d_floatOutputs, sizeof(float) * N,
                     cudaMemcpyDeviceToHost));
    *fromDeviceTime = DURATION_NANO(fromDeviceStart);
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < nRuns; i++)
    {
        long int toDeviceTime, kernelTime, fromDeviceTime;

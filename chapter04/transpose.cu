...
int main(int argc, char **argv) {
    ...
    printf("warmup         elapsed %ld nano sec\n", elaps);
    ...
    float ibnd = 2 * nx * ny * sizeof(float) / elaps;
    printf(
        "%s elapsed %ld nano sec <<< grid (%d,%d) block (%d,%d)>>> effective "
        "bandwidth %f GB\n",
        kernelName, elaps, grid.x, grid.y, block.x, block.y, ibnd);
    CHECK(cudaGetLastError());

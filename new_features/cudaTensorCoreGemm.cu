...
int main(int argc, char **argv)
{
    ...
    CHECK(cudaMallocManaged(reinterpret_cast<void**>(&A_h), sizeof(float) * M_GLOBAL * K_GLOBAL));
    CHECK(cudaMallocManaged(reinterpret_cast<void**>(&B_h), sizeof(float) * K_GLOBAL * N_GLOBAL));
    CHECK(cudaMallocManaged(reinterpret_cast<void**>(&C_h), sizeof(float) * M_GLOBAL * N_GLOBAL));
    ...
    CHECK(cudaMalloc((void**)&A, sizeof(half) * M_GLOBAL * K_GLOBAL));
    CHECK(cudaMalloc((void**)&B, sizeof(half) * N_GLOBAL * K_GLOBAL));
    CHECK(cudaMalloc((void**)&C, sizeof(float) * M_GLOBAL * N_GLOBAL));
    CHECK(cudaMalloc((void**)&D, sizeof(float) * M_GLOBAL * N_GLOBAL));
    ...
    CHECK(cudaFuncSetAttribute(compute_gemm, cudaFuncAttributeMaxDynamicSharedMemorySize, SHMEM_SZ));
    ...
    CHECK(cudaFree((void*)A_h));
    CHECK(cudaFree((void*)B_h));
    CHECK(cudaFree((void*)C_h));
    CHECK(cudaFree((void*)A));
    CHECK(cudaFree((void*)B));
    CHECK(cudaFree((void*)C));
    CHECK(cudaFree((void*)D));

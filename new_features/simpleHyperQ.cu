...
int main(int argc, char **argv) {
    ...
    int clockRate;
    CHECK(
        cudaDeviceGetAttribute(&clockRate, cudaDevAttrClockRate, cuda_device));
    ...
    // the kernel takes more time than the channel reset time on arm archs, so
    // to prevent hangs reduce time_clocks.
    ...
    CHECK(cudaMemcpy(a, d_a, sizeof(clock_t), cudaMemcpyDeviceToHost));

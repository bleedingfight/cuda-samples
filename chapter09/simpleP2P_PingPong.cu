...
int main(int argc, char **argv)
{
    ...
    printf("\nAllocating buffers (%iMB on each GPU and CPU Host) iSize = %d...\n",
           iSize>>20,iSize);

    float **d_src = new float*[ngpus];
    float **d_rcv = new float*[ngpus];
    float **h_src = new float*[ngpus];
    ...
    for (int i = 0; i < ngpus; i++)
    {
        ...
        CHECK(cudaMallocHost(reinterpret_cast<void**>(&h_src[i]), iBytes));
